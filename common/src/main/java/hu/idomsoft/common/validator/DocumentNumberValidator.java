...
public class DocumentNumberValidator implements ConstraintValidator<DocumentNumber, OkmanyDTO> {
    ...
    public boolean isValid(OkmanyDTO value, ConstraintValidatorContext context) {
        ...
        switch (value.getOkmTipus()) {
            case SZIG_TYPE :
                message = value.getOkmanySzam().matches(SZIG_PATTERN) ? "" : "meg kell felelnie a " + SZIG_PATTERN + " reguláris kifejezésnek";
                break;
            case UTL_TYPE :
                message = value.getOkmanySzam().matches(UTL_PATTERN) ? "" : "meg kell felelnie a " + UTL_PATTERN + " reguláris kifejezésnek";
                break;
            default:
                message = value.getOkmanySzam().matches(UTL_PATTERN) ? "" : "meg kell felelnie a " + OTHER_PATTERN + " reguláris kifejezésnek";
        }
